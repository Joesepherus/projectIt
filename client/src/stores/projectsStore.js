...
export class AccountStore {
         ...
         @action
         getProjectsOfAdmin(adminId){
           console.log('adminId: ', adminId);
           return axios
             .get('/api/projectsOfAdmin/' + adminId)
             .then((response) => {
         ...
         @action
         addProject(project, adminId) {
           let promise = new Promise((resolve, reject) => {
             resolve(
               axios
                 .post('/api/project', {
                   project: { ...project, adminId }
                 })
           ...
           promise.then((response) => {
             this.getProjectsOfAdmin(adminId)
             this.project = this.getProject(response.data.project._id)
           })
         }
         ...
         deleteTask(task, section) {
           ...
           let foundTaskIndex = this.project.sections[foundSectionIndex].tasks.findIndex(function (element) {
         ...
         @action
         setDialog = (state = false, type = '', content = '', index = 0, action = '') => {
           this.dialogData = {
             type: type,
             content: content,
             open: state,
